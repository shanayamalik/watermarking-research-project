...
def KeyGen(n, message_length=512, false_positive_rate=1e-9, t=3, g=None, r=None, noise_rate=None, seed=None):
    ...
    generator_matrix = GF.Random((n, k), seed=seed)
    ...
    one_time_pad = GF.Random(n, seed=seed)
    test_bits = GF.Random(num_test_bits, seed=seed)
...
def Encode(encoding_key, message=None, seed=None):
    generator_matrix, one_time_pad, test_bits, g, noise_rate = encoding_key
    n, k = generator_matrix.shape

    if message is None:
        payload = np.concatenate((test_bits, GF.Random(k - len(test_bits), seed=seed)))
    else:
        assert len(message) <= k-len(test_bits)-g, "Message is too long"
        payload = np.concatenate((test_bits, GF.Random(g, seed=seed), GF(message), GF.Zeros(k-len(test_bits)-g-len(message))))

    error = GF(np.random.binomial(1, noise_rate, n))

    return 1 - 2 * torch.tensor(payload @ generator_matrix.T + one_time_pad + error, dtype=torch.float)
