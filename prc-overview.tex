...
Note that Distinguisher is given access to both the actual $\mathsf{Model}$, as well as the distributions of watermarked and un-watermarked outputs. 

\subsection{Achieving Undetectability}

\citep{CG24} proposes a novel approach to watermarking by replacing the randomness used by the generation process in the model with codewords from a pseudorandom error-correcting code (PRC). This means that the randomness used by the model (and hence the model response) is modified in a way that is undetectable without the secret watermarking key $\mathsf{sk}$. In the next section, we discuss the definition and properties of a PRC and provide an overview of the construction in \citep{CG24}.
...
For appropriate choices of $n, g, t, r$, it can be shown that the resulting marginal distribution on $G$ is random or pseudorandom \citep{CG24}. The pseudorandomess of $G$ will allow us to apply the LPN assumption, so that the outputs look indistinguishable from random to any adversary. The parity check matrix $P$ will allow us to efficiently detect near-codewords.
...
Zero-bit decoding works by counting the number of satisfied parity checks, and checking if they are over some threshold. We expect that for $u \leftarrow {F}_2^n$, then $d(Pu,0) \approx r/2$. For a message $s$ encoded as $x=Gs \oplus e$, even after corruptions we should expect it will have small Hamming distance from $\text{Range}(G)$ so that $d(Px,0)<r/2$ (the noise $e$ is usually set to be small).
...
While LDPC codes typically utilize very small constant sparsity $t<<n$\citep{borwankar2020lowdensityparitycheck}, however, to ensure the pseudorandomness of $G$, the sparsity is set to $t=\Theta(\log n)$. Unfortunately, a belief propagation decoder does not work for noise rates beyond $O(\log t/t)$ according to \citep{CG24} which is why the simple parity check decoding is used.

In \citep{gunn2025undetectable}, the decoding function is modified to optimize for images by using a softmax function instead of bit strings, and giving more weight to values with higher magnitude. In addition, they use a constant $t = 3$, which means that the scheme is not practically undetectable against motivated adversaries. However, it is still undetectable in practice, as shown by the undetectibility discussion in the next section. 
